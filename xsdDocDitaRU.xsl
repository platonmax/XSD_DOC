...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="componentTypeLabels" as="map(xs:string, xs:string)"
        ...
        'Element'           : 'Элемент',
        'Attribute'         : 'Атрибут',
        'Complex_Type'      : 'Комплексный тип',
        'Element_Group'     : 'Группа элементов',
        'Attribute_Group'   : 'Группа атрибутов',
        'Simple_Type'       : 'Простой тип',
        'Schema'            : 'Схема',
        'Notation'          : 'Нотация',
        'Main_schema'       : 'Основная схема',
    ...
    <xsl:template name="generateMapFromIndex">
        <xsl:result-document href="{resolve-uri($mainFile, base-uri())}" indent="yes"
            ...
            <xsl:variable name="curDir">
                <xsl:call-template name="substring-before-last">
                <xsl:with-param name="string1" select="resolve-uri($mainFile, base-uri())" />
                <xsl:with-param name="string2" select="'/'" />
                </xsl:call-template>
            </xsl:variable>

            <xsl:variable name="projDir">
                <xsl:call-template name="substring-before-last">
                <xsl:with-param name="string1" select="$curDir" />
                <xsl:with-param name="string2" select="'/'" />
                </xsl:call-template>
            </xsl:variable>

            <xsl:variable name="versionPath" select="concat($projDir,'/version.xml')"/>
            <xsl:variable name="versionDoc" select="document($versionPath)"/>
            <xsl:variable name="titleDocPath" select="concat($projDir,'/Заголовок.dita')"/>
            <xsl:variable name="titleDoc" select="document($titleDocPath)"/>
            <map>
                <title><xsl:value-of select="$titleDoc" /><data name="line-break"/><xsl:value-of select="substring-before($mainFile, '.ditamap')"/>-<xsl:value-of select="$versionDoc"/>.xsd.</title>
                
                <topicref href="../Введение.dita"/>
                <topicref href="../../Нормативные_ссылки.dita"/>
                <topicref href="../Определения.dita"/>
                <topicref href="../Описание_формата.dita"/>
                <topicref href="../Логическая_модель.dita"/>
                <topicref href="../Требования.dita"/>

                <xsl:variable name="projectName" select="substring-before($mainFile,'.')"/>
                <topicref href="../Описание_структуры.dita">
                <xsl:for-each-group select="index/ref" group-by="@base">
                        <xsl:for-each select="current-group()[not(contains(@refId,'_')) and @refType='Complex_Type']">
                                    <topicref>
                                        ...
                                        <xsl:for-each select="current-group()[contains(@refId,'_')]">                                       
                                            <topicref>
                                                <xsl:variable name="fileName" select="@base"/>
                                                <xsl:variable name="fileRefId" select="@refId"/>
                                                <xsl:choose>
                    ...
                    <xsl:for-each select="current-group()[not(contains(@refId,'_')) and @refType='Element']">
                        <topicref>
                            <xsl:variable name="fileName" select="@base"/>
                            <xsl:variable name="fileRefId" select="@refId"/>
                            <xsl:choose>
    ...
    <xsl:template match="element">
        <xsl:call-template name="component">
            <xsl:with-param name="type">Элемент</xsl:with-param>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="complexType">
        <xsl:call-template name="component">
            <xsl:with-param name="type">Комплексный тип</xsl:with-param>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="simpleType">
        <xsl:call-template name="component">
            <xsl:with-param name="type">Простой тип</xsl:with-param>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="attribute">
        <xsl:call-template name="component">
            <xsl:with-param name="type">Атрибут</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="diagram">
		...
		<xsl:variable name="height" select="@height div 2.0"/>
		...
		<xsl:if test="$height &lt; 500">
		<row>
            <entry>Диаграмма</entry>
            <entry>
                    ...
                    <xsl:choose>
                        <xsl:when test="map/area" xml:space="preserve">
                                    ...
                                    <imagemap id="{$mapName}">
                                        ...
                                        <image href="{concat('img/',$mapName,'.',$imgExtension)}" width="{$width}"/>
                        ...
                        <xsl:when test="map/@name" xml:space="preserve">
                              <xsl:variable name="mapName" select="map/@name"/>
                                        ...
                                        <xsl:if test="annotations/annotation">
                                            <row>
                                                <entry namest="col1" nameend="col{2 + $optionalOn}">
													...
													<xsl:call-template name="buildAnnotation2"/>
													</xsl:for-each>
                                                </entry>
                                            </row>
                                        </xsl:if>
...
/-->
    ...
    <xsl:template match="children">
        <row>
            <entry>Подчиненные элементы</entry>
            <entry>
    ...
    <xsl:template match="source | instance">
        <xsl:if test="sum(child::node()/string-length()) &lt; 2000">
            <row>
                <entry>
                    <xsl:choose>
                        <xsl:when test="compare(local-name(.), 'source') = 0">Описание в XSD</xsl:when>
                        <xsl:otherwise>Структура</xsl:otherwise>
                    </xsl:choose>
                </entry>
                <entry>
    ...
    <xsl:template name="buildAnnotation">
        <xsl:variable name="tokens" select="token"/>
        <xsl:call-template name="formatAnnotation">
            <xsl:with-param name="tokens" select="$tokens"/>        
        </xsl:call-template>
    </xsl:template>


    <xsl:template name="formatAnnotation">
        <xsl:param name="tokens"/>
        <xsl:choose>
            <xsl:when test="$tokens[text() = '&lt;text']">
                <xsl:for-each select="$tokens">
                        <xsl:choose>
                            <xsl:when test="text() = '&lt;text'">
                                <row>
                                    <entry>Описание</entry>
                                    <entry align="justify"><xsl:value-of select="following-sibling::*[2]"/></entry>
                                </row>
                            </xsl:when>
                            <xsl:when test="text() = '&lt;formula'">
                                <row>
                                    <entry>Формула расчета</entry>
                                    <entry align="justify"><lines><xsl:value-of select="following-sibling::*[2]"/></lines></entry>
                                </row>
                            </xsl:when>
                            <xsl:when test="text() = '&lt;links'">
                                <row>
                                    <entry>Нормативные документы</entry>
                                    <entry align="justify"><lines><xsl:value-of select="following-sibling::*[2]"/></lines></entry>
                                </row>
                            </xsl:when>
                        </xsl:choose>
    ...
    <xsl:template name="buildAnnotation2">
        <xsl:variable name="tokens" select="token"/>
        <xsl:call-template name="formatAnnotation2">
            <xsl:with-param name="tokens" select="$tokens"/>        
        </xsl:call-template>
    </xsl:template>


    <xsl:template name="formatAnnotation2">
        <xsl:param name="tokens"/>
        <xsl:choose>
            <xsl:when test="$tokens[text() = '&lt;text']">
                <xsl:for-each select="$tokens">
                        <xsl:choose>
                            <xsl:when test="text() = '&lt;text'">
								<lines><xsl:value-of select="following-sibling::*[2]"/></lines>
                            </xsl:when>
                            <xsl:when test="text() = '&lt;formula'">
								<b>Формула расчета:</b> <lines><xsl:value-of select="following-sibling::*[2]"/></lines>
                            </xsl:when>
                            <xsl:when test="text() = '&lt;links'">
								<b>Нормативные документы:</b> <lines><xsl:value-of select="following-sibling::*[2]"/></lines>
                            </xsl:when>
                        </xsl:choose>
            </xsl:for-each>
            </xsl:when>
